...
class PatrocinadorMySqlDAO implements PatrocinadorDAO{
	...
	public function insert($patrocinador){
		$sql = 'INSERT INTO patrocinador (imagem, datainicial, datafinal, acessos, qtd_acessos, id_categoria) VALUES (?, ?, ?, ?, ?, ?)';
		$sqlQuery = new SqlQuery($sql);
		...
		$sqlQuery->setNumber($patrocinador->idCategoria);
	...
	public function update($patrocinador){
		$sql = 'UPDATE patrocinador SET imagem = ?, datainicial = ?, datafinal = ?, acessos = ?, qtd_acessos = ?, id_categoria = ? WHERE id = ?';
		$sqlQuery = new SqlQuery($sql);
	...
	public function queryByIdCategoria($value){
		$sql = 'SELECT * FROM patrocinador WHERE id_categoria = ?';
		$sqlQuery = new SqlQuery($sql);
		$sqlQuery->setNumber($value);
		return $this->getList($sqlQuery);
	}
	...
	public function deleteByIdCategoria($value){
		$sql = 'DELETE FROM patrocinador WHERE id_categoria = ?';
		$sqlQuery = new SqlQuery($sql);
		$sqlQuery->setNumber($value);
		return $this->executeUpdate($sqlQuery);
	}
	...
	protected function readRow($row){
		...
		$patrocinador->idCategoria = $row['id_categoria'];
