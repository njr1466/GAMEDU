...
$app->run();
	...
	function queryAll(){
	    if(!autenticacao::autenticar()){
                ...
                $turma = DAOFactory::getTurmaDAO()->queryAll();
	        echo json_encode($turma);
	    }
        }
	...
	function queryAllOrderBy($orderColumn){
            ...
            if(!autenticacao::autenticar()){
                ...
                $turma = DAOFactory::getTurmaDAO()->queryAllOrderBy($orderColumn);
	        echo json_encode($turma);	
            }    
	}
	...
	function delete($id){
            if(!autenticacao::autenticar()){
                ...
                $turma = DAOFactory::getTurmaDAO()->delete($id);
	...
	function insert(){
            if(!autenticacao::autenticar()){
                ...
                $turma = DAOFactory::getTurmaDAO()->insert($turma);
                echo json_encode($turma);
             }
	}
	...
	function update(){
            if(!autenticacao::autenticar()){
                ...
                $request = \Slim\Slim::getInstance()->request();
                $turma = json_decode($request->getBody());
                $turma = DAOFactory::getTurmaDAO()->update($turma);
